    ...
    <div className="min-h-screen bg-base-200">
      <nav className="navbar bg-base-100 shadow">
        ...
        <div className="flex-none gap-2 px-4">
          <button
            className={`btn btn-sm ${page === 'words' ? 'btn-primary' : 'btn-ghost'}`}
            onClick={() => setPage('words')}
          >
            단어 관리
          </button>
          <button
            className={`btn btn-sm ${page === 'stats' ? 'btn-primary' : 'btn-ghost'}`}
            onClick={() => setPage('stats')}
          >
            통계
          </button>
          <button
            className={`btn btn-sm ${page === 'admins' ? 'btn-primary' : 'btn-ghost'}`}
