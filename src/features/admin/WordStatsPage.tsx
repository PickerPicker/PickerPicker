...
export function WordStatsPage() {
  ...
  const [sort, setSort] = useState<'exposure_desc' | 'accuracy_asc' | 'accuracy_desc'>(
    'exposure_desc',
  )
    ...
    <div>
      ...
      {overview && (
        <div className="stats shadow mb-4 w-full">
          <div className="stat">
            <div className="stat-title">총 플레이어</div>
            <div className="stat-value">{overview.total_players}</div>
          </div>
          <div className="stat">
            <div className="stat-title">총 게임 세션</div>
            <div className="stat-value">{overview.total_sessions}</div>
          </div>
          <div className="stat">
            <div className="stat-title">활성 단어</div>
            <div className="stat-value">{overview.active_word_count}</div>
          </div>
          <div className="stat">
            <div className="stat-title">평균 점수</div>
            <div className="stat-value">{overview.avg_score.toFixed(0)}</div>
          </div>
        </div>
      )}
      <div className="flex gap-2 mb-2">
        <select
          ...
          value={sort}
          onChange={(e) =>
            setSort(e.target.value as 'exposure_desc' | 'accuracy_asc' | 'accuracy_desc')
          }
        >
