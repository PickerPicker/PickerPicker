...
export function TutorialStage({
  ...
  const applyJudgment = useCallback(
    (type: JudgmentType, note: SpawnedNote | null) => {
      ...
      if (note) {
        ...
        note.hitType = type
        setNotes((prev) =>
          prev.map((n) => (n.id === note.id ? { ...n, hit: true, hitType: type } : n)),
        )
      }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      >
        {cleared ? (
          <span className="text-2xl font-black tracking-wide text-success">✓ 잘했어요!</span>
        ) : (
          <>
            <span
            ...
            {progressLabel && (
              <span className="font-mono text-base font-bold text-primary">{progressLabel}</span>
            )}
          </>
        )}
      </div>
