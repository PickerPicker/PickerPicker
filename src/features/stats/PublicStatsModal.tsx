        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {status === 'public' && data && (
            <>
              ...
              {/* motto */}
              {data.motto && (
                <p className="text-sm italic text-center px-2" style={{ color: '#f0abfc' }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                <StatCell
                  label="최고 점수"
                  value={(data.totals?.best_score ?? 0).toLocaleString()}
                />
                ...
                <StatCell
                  label="평균 점수"
                  value={(data.averages?.avg_score ?? 0).toLocaleString()}
                />
              ...
              {/* 세부 평균 지표 */}
              {data.averages &&
                (data.averages.median_score != null || data.averages.avg_stage != null) && (
                  <Section title="세부 기록">
                    <div className="grid grid-cols-2 gap-2">
                      {data.averages.median_score != null && (
                        <StatCell
                          label="중앙값 점수"
                          value={data.averages.median_score.toLocaleString()}
                        />
                      )}
                      {data.averages.min_score != null && (
                        <StatCell
                          label="최저 점수"
                          value={data.averages.min_score.toLocaleString()}
                        />
                      )}
              ...
              {data.trend && (
                <Section title="최근 활동">
                  <div className="grid grid-cols-2 gap-2">
                    <StatCell
                      label="7일 평균 점수"
                      value={(data.trend.last_7_days_avg_score ?? 0).toLocaleString()}
                    />
                    <StatCell
                      label="30일 평균 점수"
                      value={(data.trend.last_30_days_avg_score ?? 0).toLocaleString()}
                    />
                    <StatCell
                      label="7일 플레이"
                      value={`${data.trend.last_7_days_play_count ?? 0}회`}
                    />
                    <StatCell
                      label="30일 플레이"
                      value={`${data.trend.last_30_days_play_count ?? 0}회`}
                    />
                  </div>
                </Section>
              )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    ...
    >
      <h3 className="text-xs font-bold tracking-wide" style={{ color: '#c084fc' }}>
        {title}
      </h3>
      {children}
    </div>
