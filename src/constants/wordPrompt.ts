// AI에게 줄 단어 JSON 생성 프롬프트.
// 규칙은 docs/design/06_스테이지_데이터셋.md 의 "추가 스테이지 생성 프롬프트"(검증된 11개 규칙)를
// admin 단일 단어 등록 형식(snake_case 단일 객체)으로 변환한 것이다.
// docs/단어_JSON_생성_프롬프트.md 와 동일 내용 — 둘 중 하나 수정 시 함께 맞춘다.
// 맨 끝 {WORD} 플레이스홀더가 관리자가 입력한 단어로 치환된다.
export const WORD_PROMPT_TEMPLATE = `너는 리듬 타이핑 게임 "PickerPicker"의 단어 데이터 생성기야.
...
// 단어를 끼워 넣어 최종 프롬프트를 만든다. 단어가 비면 ○○ placeholder 유지.
export function buildWordPrompt(word: string): string {
  const filled = word.trim() || '○○'
  return WORD_PROMPT_TEMPLATE.replace('{WORD}', filled)
}
